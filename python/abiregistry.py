...
class ABIRegistry:
    ...
    async def _load_all_abis(self, base_path: Optional[Path] = None) -> None:
        ...
        tasks = [self._load_and_validate_abi(abi_type, abi_dir / filename, critical_abis)
    ...
    async def _load_and_validate_abi(self, abi_type: str, abi_path: Path, critical_abis: set) -> None:
        """
        Load a single ABI file and validate its content.
        """
        try:
    ...
    def _validate_abi(self, abi: List[Dict[str, Any]], abi_type: str) -> bool:
        ...
        missing = self.REQUIRED_METHODS.get(abi_type, set()) - found_methods
        if missing:
            logger.error(f"ABI for {abi_type} missing required methods: {missing}")
            return False
        return True
