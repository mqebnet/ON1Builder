...
class APIConfig:
    ...
    def __init__(self, configuration: Configuration) -> None:
        ...
        self.apiconfigs: Dict[str, Dict[str, Any]] = {
            ...
            "uniswap_subgraph": {
                "base_url": f"https://gateway.thegraph.com/api/{os.getenv('GRAPH_API_KEY','')}/subgraphs/id/{os.getenv('UNISWAP_V2_SUBGRAPH_ID','EYCKATKGBKLWvSfwvBjzfCBmGwYNdVkduYXVivCsLRFu')}",
                "rate_limit": 5,
                "weight": 0.3,
                "success_rate": 1.0,
            },
    ...
    async def initialize(self) -> None:
        ...
        logger.debug("APIConfig initialized with %d tokens.", len(self.token_symbol_to_address))
    ...
    def _get_api_symbol(self, symbol_or_address: str, api: str) -> str:
        ...
        return self.symbol_to_api_id.get(symbol_or_address.upper(), symbol_or_address.upper() if api != "coingecko" else symbol_or_address.upper().lower())
            ...
            for attempt in range(self.MAX_REQUEST_ATTEMPTS):
                try:
                    ...
                    async with self.session.get(url, params=params, headers=headers, timeout=timeout) as resp:
                        if resp.status == 429:
                            logger.warning("429 from %s; backing off", provider)
                    ...
                    logger.exception("Unexpected error on %s: %s", provider, e)
                    await asyncio.sleep(self.REQUEST_BACKOFF_FACTOR ** attempt)
    ...
    async def _fetch_price(self, provider: str, token: str, vs: str) -> Optional[Decimal]:
        ...
        try:
            ...
            if provider == "uniswap_subgraph":
                query = {"query": "query ($id: String!) { token(id: $id) { derivedETH } }",
                         "variables": {"id": self.get_token_address(token).lower()}}
                await self.make_request(provider, cfg["base_url"])
                return None
    ...
    async def _fetch_historical_prices(self, provider: str, token: str, days: int) -> Optional[List[float]]:
        ...
        try:
            ...
            if provider == "cryptocompare":
                ...
                return None
            if provider == "uniswap_subgraph":
                query = {"query": "query ($token: String!, $d: Int!) { tokenDayDatas(first: $d, orderBy: date, orderDirection: desc, where:{token:$token}) { priceUSD } }",
            ...
            logger.debug("Historical price error [%s]: %s", provider, e)
            return None
    ...
    async def fetch_historical_prices(self, token_or_addr: str, days: int = 30) -> List[float]:
        ...
        for provider in self.apiconfigs:
            ...
            if hp:
                ...
                return hp
        logger.info("No historical prices for %s", token_norm)
        return []
    ...
    async def train_price_model(self) -> None:
        try:
            ...
            if len(df) < self.configuration.MIN_TRAINING_SAMPLES:
                ...
                return
            X = df[["price_usd", "volume_24h", "market_cap", "volatility", "liquidity_ratio", "price_momentum"]].fillna(0)
            y = df["price_usd"].fillna(0)
            mdl = LinearRegression().fit(X, y)
            joblib.dump(mdl, self.configuration.MODEL_PATH)
            logger.info("Price model trained with %d rows.", len(df))
        except Exception as e:
            logger.exception("Failed to train price model: %s", e)
