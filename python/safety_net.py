...
class Safety_Net:
    ...
    def __init__(
        ...
        self.price_cache: TTLCache = TTLCache(maxsize=2000, ttl=self.configuration.SAFETY_NET_CACHE_TTL) # Use configurable CACHE_TTL
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # self.price_cache = TTLCache(maxsize=1000, ttl=self.configuration.SAFETY_NET_CACHE_TTL)

            # Initialize gas price cache (already done in __init__, no need to repeat unless TTL needs reset)
            # self.gas_price_cache = TTLCache(maxsize=1, ttl=self.configuration.SAFETY_NET_GAS_PRICE_TTL)
    ...
    async def assess_transaction_risk(
        ...
        try:
            ...
            logger.error(f"Error in risk assessment: {e}", exc_info=True) # Include traceback
            return 0.0, {}
    ...
    async def get_dynamic_gas_price(self) -> Decimal:
        ...
        try:
            ...
            logger.error(f"Error fetching dynamic gas price: {e}", exc_info=True) # Include traceback
