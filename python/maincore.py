...
class MainCore:
    ...
    async def _initialize_web3(self) -> Optional[AsyncWeb3]:
        ...
        for name, provider in providers:
            for _ in range(self.configuration.get_config_value("WEB3_MAX_RETRIES", 3)):
                try:
    ...
    async def _get_providers(self) -> List[tuple]:
        ...
        if ws:
            providers.append(("ws", WebSocketProvider(ws)))
    ...
    async def initialize_components(self) -> None:
        ...
        mempoolmonitor = MempoolMonitor(self.web3, safetynet, noncecore, apiconfig, await self.configuration.get_token_addresses(), self.configuration, marketmonitor)
